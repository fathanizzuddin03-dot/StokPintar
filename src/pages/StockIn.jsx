...
export default function StockIn() {
  ...
  const handleSubmit = async () => {
    ...
    toast({ title: 'Barang masuk berhasil', description: `${prod.name} +${qty}` });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-heading font-bold text-gray-900">Barang Masuk</h1>
          <p className="text-sm text-gray-500 mt-1">Input penerimaan barang ke gudang utama</p>
        </div>
        <Button onClick={() => setShowForm(!showForm)}><Plus className="w-4 h-4 mr-2" /> Input Barang Masuk</Button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200">
        {movements.length === 0 ? (
          <div className="text-center py-16"><ArrowDownToLine className="w-12 h-12 text-gray-300 mx-auto mb-3" /><p className="text-gray-500">Belum ada barang masuk</p></div>
        ) : (
